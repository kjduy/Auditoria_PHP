...
reserv_table = $('#od_table').DataTable({  
    "ajax":{            
        "url": "../controller/patient_od_table.php", 
        "method": 'POST',
        "data": {id:$('#id_HC').val()},
        "dataSrc":""
    },
...
$(document).on("click", ".btnEditar", function(){		        
    ...
    window.location.href='AppointmentOMedical.php?link=patientAppointmentHO.php&code='+code+'&msg='+id;
});
...
$(document).on("click", ".btnMirar", function(){		        
    ...
    let code = $('#id_HC').val(); // el id del paciente, que sirve para recargar la pagina        
    id = parseInt(fila.find('td:eq(0)').text()); //capturo el ID de la cita medica
    window.location.href='AppointmentOView.php?&link=patientAppointmentHO.php&code='+code+'&msg='+id;
});
